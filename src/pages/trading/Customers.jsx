...
const Customers = () => {
  ...
  const fetchCustomerTransactions = async (customerId, filter, custom) => {
    setTxLoading(true);
    const { from, to } = getTxDateRange(filter || txDateFilter, custom || txCustomDate);
    const { data } = await getSalesByCustomer(customerId, from, to);
  ...
  const handleTxDateFilterChange = (val) => {
    setTxDateFilter(val);
    if (selectedCustomer && val !== 'custom') {
      fetchCustomerTransactions(selectedCustomer.id, val, txCustomDate);
    }
  };
  ...
  const handleTxCustomDateChange = (val) => {
    ...
    if (selectedCustomer && val) {
      fetchCustomerTransactions(selectedCustomer.id, 'custom', val);
    }
  };
  ...
  const handleView = (customer) => {
    ...
    fetchCustomerTransactions(customer.id, 'all', '');
  };
          ...
          <div className="bg-white p-4 rounded-lg border shadow-sm mt-4">
            ...
            <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg mb-3">
              <div className="text-center flex-1 border-r border-green-200">
                <div className="text-xs text-green-800">Số hóa đơn</div>
            ...
            <div className="space-y-2">
              {txLoading ? (
                ...
                <div className="text-center py-4 text-gray-500 italic">Không có hóa đơn nào.</div>
              ) : transactions.map(tx => (
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-200">
                      ...
                      <div className="flex items-center gap-1 text-xs font-medium">
                        {Number(tx.paid_amount) >= Number(tx.total_amount) ? (
                          <span className="text-green-600"><i className="fas fa-check-circle"></i> Đã thanh toán</span>
                        ) : (
                          <span className="text-orange-500"><i className="fas fa-clock"></i> Đã thu {Number(tx.paid_amount || 0).toLocaleString()}đ</span>
                        )}
                      </div>
                    </div>
                  </div>

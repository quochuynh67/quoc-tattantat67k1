...
// ─────────────────────────────────────────────────────────────────────────────
// CÂU HỎI MẪU — thêm / sửa tại đây, format: { q: "câu hỏi", a: "câu trả lời" }
// ─────────────────────────────────────────────────────────────────────────────
const SAMPLE_QA = [

  {
    q: "Số điện thoại chành xe Quốc Huỳnh là gì?",
    a: "Số điện thoại chành xe Quốc Huỳnh là 0762985555 đó bạn~",
  },
  {
    q: "Tôi vừa thấy 1 chú cún bị lạc tại X, có ai hay gần đây bị mất cún không?",
    a: "Để cô hỏi thăm giùm bạn nha~",
  },
  {
    q: "Tôi bị rơi cái ví ở đường về nhà tại khu phố X",
    a: "À, đã có người nhặt ở đây nè cưng~ để cô hỏi thăm giùm bạn nha~ may quá họ nhặt được đó",
  },
  {
    q: "Đặc sản nổi tiếng của Phú Tân là gì?",
    a: "Phú Tân nổi tiếng với mắm, cá khô, bánh bò thốt nốt và các món miền Tây đặc trưng đó nha~",
  },
  {
    q: "Có gia đình nào khó khăn cần giúp đỡ không?",
    a: "Có chứ, để cô xem giúp gia đình cô X ở xã Y nha cưng~",
  },
  {
    q: "Ở Phú Tân có chùa hay di tích nào đẹp không?",
    a: "Phú Tân có chùa Phước Long, đình thần và nhiều điểm văn hóa đặc trưng miền Tây đó bạn~",
  },
];
// ─────────────────────────────────────────────────────────────────────────────
...
const SampleQADisplay = ({ onClickQuestion }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => goTo((idx + 1) % SAMPLE_QA.length), 5500);
          ...
          >
            ...
            <Paper
              ...
              sx={{
                ...
                boxShadow: isUser ? "none" : "0 4px 20px rgba(0,0,0,0.1)",
                lineHeight: 1.6,
              }}
            >
              {msg.content}
...
export default function HeroAskSection() {
  ...
  const send = async (questionOverride) => {
    ...
    if (!question || loading || isCooling) return;
    ...
    let started = false;
    try {
      await streamAgent(next, "general", HERO_AGENT, (chunk) => {
        if (!started) {
          started = true;
          setLoading(false);
    ...
    >
      <Container maxWidth="md" sx={{ position: "relative", zIndex: 1 }}>
        ...
        >
          ...
          <IconButton
            onClick={() => send()}
            disabled={!input.trim() || loading || isCooling}
            sx={{
              bgcolor: HERO_AGENT.color, color: "#fff",
              width: 44, height: 44,
              transition: "all 0.2s",
        ...
        {/* Sample Q&A or conversation */}
        {!hasConversation ? (
          <SampleQADisplay onClickQuestion={handleClickSample} />
        ) : (
